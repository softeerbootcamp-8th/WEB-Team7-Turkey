...
const validFields: CustomerSignupFields = {
  loginId: 'customer1',
  password: 'password',
  name: '홍길동',
...
describe('validateCustomerSignup', () => {
  it('중복 확인과 휴대전화 인증을 완료하고 필수값이 유효하면 오류가 없다', () => {
    expect(validateCustomerSignup(validFields, {
...
describe('서버 오류 분류', () => {
  it('동시 가입으로 발생한 아이디 중복을 해당 필드 오류로 분류한다', () => {
    expect(classifyCustomerSignupError(httpError(409, '이미 사용 중인 아이디입니다.'))).toEqual({
      target: 'loginId',
  ...
  it('인증 토큰 만료를 휴대전화 필드 오류로 분류한다', () => {
    expect(classifyCustomerSignupError(httpError(400, '휴대전화 인증이 필요하거나 인증이 만료되었습니다.')))
      .toEqual({
        target: 'phoneNumber',
