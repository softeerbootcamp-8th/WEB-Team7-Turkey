...
export function LocationSimulationControl({
  ...
  if (!expanded) {
    return (
      <button
        ...
        className={`fixed right-3 top-20 z-50 flex h-11 w-11 items-center justify-center rounded-full border border-surface-container shadow-md transition-colors ${simulator.enabled ? 'bg-primary-container text-on-primary-container' : 'bg-surface-container-lowest text-on-surface'}`}
      >
  ...
  return (
    <div className="fixed right-3 top-20 z-50 flex flex-col items-end gap-1">
      <div
        className="flex h-11 items-center gap-1.5 rounded-full border border-surface-container bg-surface-container-lowest px-2 shadow-md"
      ...
      >
        ...
        >
          <span
            aria-hidden="true"
            className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${simulator.enabled ? 'translate-x-4' : 'translate-x-0.5'}`}
          />
        </button>
        <input
          type="range"
