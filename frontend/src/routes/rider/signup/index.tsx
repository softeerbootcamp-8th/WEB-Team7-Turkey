...
import { useRiderSignup } from '@/api/generated/rider-signup/rider-signup'
import {
...
const inputClassName = 'w-full h-[52px] px-4 bg-surface-container-lowest border border-surface-container rounded-xl text-body-lg font-body-lg text-on-surface outline-none focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors placeholder:text-surface-dim disabled:bg-surface-container-low disabled:text-on-surface-variant'
const secondaryButtonClassName = 'h-[52px] px-4 bg-surface-container border border-surface-container text-on-surface font-label-lg text-label-lg rounded-xl hover:bg-surface-variant transition-colors whitespace-nowrap disabled:cursor-not-allowed disabled:opacity-60'

function RiderSignup() {
  const router = useRouter()
  const [fields, setFields] = useState<RiderSignupFields>(initialFields)
  const [fieldErrors, setFieldErrors] = useState<RiderSignupFieldErrors>({})
  ...
  function updateField(field: keyof RiderSignupFields, value: string) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const errors = validateRiderSignup(fields, {
      checkedLoginId,
    ...
    <div className="max-w-md mx-auto min-h-screen bg-surface-container-lowest relative flex flex-col shadow-sm">
      ...
      <form className="flex-1 flex flex-col" onSubmit={handleSubmit} noValidate>
        <main aria-label="라이더 회원가입" className="flex-1 px-container-margin pt-lg pb-32 flex flex-col gap-xl">
            ...
            <div className="flex flex-col gap-2">
              ...
              {phoneStatus === 'verified' && (
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button type="button" className="text-label-md text-on-surface-variant underline" onClick={resetPhoneVerification}>
                    번호 변경
                  </button>
                </div>
              )}
          ...
          <section className="rounded-xl border border-surface-container bg-surface-bright p-5 flex flex-col gap-2">
