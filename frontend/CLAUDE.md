...
- 로그인 성공 시 역할 힌트(`CUSTOMER`/`RIDER`)를 `localStorage`에 저장하고, 이후에는 **해당 역할의 세션
  API 하나만 조회**한다(#288). 역할 힌트가 없으면 세션 API를 호출하지 않고 비로그인으로 판정한다.
  이 값은 조회 대상 선택용일 뿐 인증·인가 근거가 아니며, 401·로그아웃 시 세션 캐시와 함께 제거한다.
- 역할 무관 `GET /api/session`이 생기면 역할 힌트를 제거하고 서버 응답의 역할을 정본으로 사용한다.
