...
- **경로 탐색(OSRM) 클라이언트가 `common/routing` 에 생겼다(#420, 2026-08-07).** 호출 계층만이고
  호출자는 아직 없다 — 추적 응답에 ETA·경로를 채우는 것은 #421 이다. 확정된 것: 타임아웃 연결
  300ms / 읽기 700ms, 재시도 없음, 실패는 예외 대신 빈 값, 연속 3회 실패 시 호출 스킵(대기 1초 →
  2배씩 → 30초 상한, 성공 시 초기화, TCP 재전송 오마주 — 사람 확인), `overview=simplified`
  (사람 확인, MVP 판단). 남은 미결:
  - **4xx 는 회로 차단의 실패로 세지 않는다.** OSRM 은 경로를 못 찾아도 400(`NoRoute`·`NoSegment`)을
    주는데, 서울시 전용 extract 라 서울 밖 좌표는 정상적으로 생긴다 — 장애로 세면 그런 주문 세 건이
    모든 고객의 ETA 를 끊는다. 서비스 지역이 넓어지면 이 구분을 다시 본다
  - 타임아웃·백오프 값과 `simplified` 상세도가 전부 **실측 없이 정한 잠정값**이다. 부하 테스트와
    화면(#422)에서 재검토
  - **로컬·CI 어디에도 실제 OSRM 에 붙는 테스트가 없다**(배포된 OSRM 은 WAS 보안그룹에서만 접근
    가능, #416). #421 에서 로컬 PoC 컨테이너(`osrm-seoul-city`, #407)에 직접 질의해 **응답 구조가
    `OsrmResponse` 매핑과 일치**하는 것은 확인했지만, **배포 서버 자체는 여전히 미확인**이다
...
- **추적 스냅샷이 ETA 를 싣는다(#421, 2026-08-07).** 출발점은 Redis 최신 위치, 도착점은 상태별로
  ...
  **예상 경로 좌표는 싣지 않는다**(사람 확인, 2026-08-07 — 한 번 구현했다가 걷어냈다). 고객에게
  ETA 만 보여주기로 했다. 라우팅 응답에 경로가 함께 오므로 **OSRM 호출 수·지연은 같고**, 필요해지면
  ...
  - **경로 결과 캐시가 없어 조회마다 OSRM 을 부른다.** 백업 폴링 1분 주기 × 동시 추적 수만큼 호출된다
  ...
  - `DeliveryTrackingQueryService.getTracking` 에서 **`@Transactional` 을 걷어냈다** — 외부 HTTP
    호출(최대 1초)이 DB 커넥션을 잡지 않게 하려는 것이고, 대신 지연 로딩을 쓸 수 없어
