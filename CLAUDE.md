...
- 통합/E2E 테스트가 Redis를 인메모리 대체(`InMemoryVerificationCodeStore`, `InMemoryRiderLocationStore`)로만
  검증함 — 실제 Redis TTL 만료 동작은 아직 검증되지 않음(#20, #81). #81 에서 컨테이너에 직접 붙여
  `SET+EX` 로 TTL 600초가 걸리는 것까지는 일회성으로 확인했지만(작업 기록 참고), 만료 자체와
  영구 테스트는 없다. Redis를 쓰는 테스트를 어느 층에 둘지 또는 CI에 Redis 서비스 컨테이너를
  붙일지 결정이 필요하다
