...
- **Redis Streams 사용 금지**
- **Redis Pub/Sub 은 SSE 이벤트 팬아웃 용도로만 허용**(2026-07-29 결정,
  [Discussion #246](https://github.com/softeerbootcamp-8th/WEB-Team7-Turkey/discussions/246)).
  수평 확장으로 위치를 처리한 인스턴스와 고객의 SSE 연결을 들고 있는 인스턴스가 달라질 수 있어
  팬아웃 수단이 필요해졌다. **다른 용도(작업 큐, 도메인 이벤트 버스, 인스턴스 간 RPC)로 확장하지
  않는다** — 그건 별도 논의가 필요하다. Streams 금지는 유지한다: 브로드캐스트가 필요한데 컨슈머
  그룹은 작업 분배 의미론이고, 재연결을 최신 스냅샷으로 복구하기로 정해(#79) 영속·재생이 사 줄 것이 없다.
...
- Redis 용도는 5가지로 한정: **세션 저장 / 라이더 최신 위치 / GEO 위치 검색 / 휴대전화 인증번호(TTL) /
  SSE 이벤트 팬아웃(Pub/Sub)**. 영속 원본 저장소로 쓰지 않는다(#20 작업 중 4번째 확장,
  `docs/worklog/2026-07-28-20-phone-verification-request.md` 참고. 5번째는 2026-07-29 수평 확장 결정,
  Discussion #246).
...
- 실시간 라이더 위치 전달은 **SSE** 사용(Polling 아님). 위치가 실제로 변경됐을 때만 이벤트 전송.
  - 다중 인스턴스이므로 **Redis Pub/Sub 으로 팬아웃한다**(2026-07-29 결정, Discussion #246).
    위치를 처리한 인스턴스가 발행하고, 모든 인스턴스가 구독해 **해당 주문의 `SseEmitter` 를 들고 있는
    인스턴스만 실제로 전송**한다. 위치를 처리한 인스턴스와 고객의 SSE 연결이 붙은 인스턴스가 다를 수
    있어 직접 전달이 불가능하기 때문이다.
    - `SseEmitter` 레지스트리는 **인스턴스 로컬(인메모리)로 둔다.** emitter 는 그 JVM 의 열린 응답에
      묶여 있어 공유할 수 없다. 이건 "상태를 JVM 에 두지 않는다" 규칙의 정당한 예외다.
    - 유실은 감내한다(at-most-once). 위치 이벤트는 최신 값 스트림이라 한 건 유실되면 다음 이벤트(BUSY
      5초 주기)와 재연결 시 스냅샷(#79)이 복구한다. 그래서 Streams 의 영속·재생이 필요 없다.
    - **주문의 중복 연결 수 제한(#77)은 Redis 카운터로 센다.** 같은 주문의 연결이 여러 인스턴스에
      흩어지므로 인스턴스 로컬 레지스트리로는 총 개수를 알 수 없다.
      - **정수 카운터(`INCR`/`DECR`)를 쓰지 않는다**(#77 구현, 2026-07-30). 정수 하나로는 "살아 있는
        보유자"와 "죽은 보유자"를 구분할 수 없어, 인스턴스가 비정상 종료하면 그 배송이 **영구히
        구독 불가**가 된다 — 이 저장소는 `systemctl restart` + graceful shutdown 미설정이라
        **배포마다 확정적으로** 일어난다. TTL 로도 못 고친다(갱신하면 누수가 영구화, 갱신하지 않으면
        정상 연결까지 날아간다). **ZSET(member=emitterId, score=마지막 생존 확인 시각) + Lua** 로
        두어 stale 기록이 스스로 걷어내지게 하고, 오차가 과소 집계(상한이 느슨해짐) 쪽으로
        떨어지게 한다. 구현은 `location/sse/RedisTrackingConnectionLimiter`.
      - **그 자기치유는 heartbeat 가 있어야 성립한다.** SSE 는 쓰기를 시도할 때만 끊김을 알 수 있어서,
        heartbeat 없이는 탭을 닫은 클라이언트가 emitter 타임아웃까지 자리를 잡고 있다.
- 라이더 상태와 배송 상태를 분리한다.
- 구조는 모놀리식 Spring Boot WAS이고 **여러 대로 수평 확장한다**(2026-07-29 요구사항 변경. 그 전에는
  단일 인스턴스 전제였다). 코드 수준에서만 책임 분리하며 MSA는 아니다 — 수평 확장은 같은 모놀리스를
  여러 대 띄우는 것이라 MSA와 무관하다.
  - **상태를 JVM에 두지 않는다.** 세션·라이더 최신 위치·인증번호는 Redis, 정합성이 필요한 것은 MySQL이
    정본이다. 그래서 스티키 세션이 필요 없다. 인메모리 캐시·`static` 가변 컬렉션·인스턴스 로컬 카운터를
    새로 만들면 인스턴스마다 값이 갈린다 — 리뷰 시 확인할 것.
    - **유일한 예외는 `SseEmitter` 레지스트리다**(위 SSE 항목). emitter 는 그 JVM 의 열린 HTTP 응답에
      묶여 있어 공유가 불가능하다. 그래서 이벤트를 Pub/Sub 으로 모든 인스턴스에 뿌리고, 자기 것을 들고
      있는 인스턴스만 전송한다. **연결 개수 같은 집계값은 이 예외에 포함되지 않는다** — Redis 로 센다.
  - 롤링 배포 중 구·신 버전이 공존하므로 **Flyway 마이그레이션은 이전 버전 코드와 호환돼야 한다**
    (컬럼 추가 후 사용, 사용 중단 후 삭제). `ddl-auto: validate` 때문에 호환되지 않으면 구 버전
    인스턴스가 재시작에서 기동 실패한다.
...
- 운행 종료 후 최대 10분간 라이더 최신 위치가 Redis 에 남는다(#81). `RiderLocationStore` 에
  `delete` 를 두지 않았고 운행 상태 변경 API 도 없어서, TTL 만료까지 배차 후보 검색(#83)에 잡힌다.
...
- EC2 구성 및 MySQL 배치 방식(EC2 직접 설치 vs RDS). Redis는 ElastiCache로 결정됨(위 「확정된 결정」).
  **수평 확장 요구사항(2026-07-29)에 따라 MySQL을 EC2에 직접 설치하는 안은 재검토 필요** — 앱 인스턴스가
  늘면 어느 인스턴스에 붙일지, 그 인스턴스가 죽으면 DB도 함께 죽는 문제가 생긴다.
- **다중 인스턴스 배포 구성 전반이 미결**(2026-07-29 요구사항 변경). 정해야 하는 것: 로드밸런서(ALB)
  도입과 CloudFront `/api/*` origin을 EC2 단일 → ALB로 전환, 인스턴스 수·오토스케일링 여부,
  `.github/workflows/deploy.yml`의 무중단·롤링 배포 재작성(현재는 `self-hosted` 러너에서
  `systemctl restart` — **단일 호스트 전제**), ALB idle timeout이 SSE heartbeat 간격보다 길어야 함.
  SSE 전달 방식 결정([Discussion #246](https://github.com/softeerbootcamp-8th/WEB-Team7-Turkey/discussions/246))과
  맞물리므로 그것부터 정한다.
