...
- 세션 TTL은 **2시간이고 인증을 통과한 요청마다 슬라이딩 갱신된다**(#439, 2026-08-11 — #27의 "고정 TTL, 슬라이딩 없음"을 뒤집은 결정). TTL 값은 `SessionStore.DEFAULT_TTL` 하나뿐이고, 두 세션 인터셉터가 `extend()`로 Redis TTL만 다시 건다.
  ...
  - 대가: **탭·앱을 열어 두면 세션이 사실상 만료되지 않는다**(프론트 세션 확인 폴링, BUSY 라이더의 5초 위치 전송이 그대로 활동 신호가 된다). 그 방어선으로 **절대 수명 상한 24시간**을 추가했다(`SessionStore.ABSOLUTE_TTL`, #562, 2026-08-18 사람 확인). 슬라이딩(`extend()`)과 무관하게 로그인 시각(`SessionValue.createdAt`) 기준 24시간이 지나면 두 세션 인터셉터가 요청마다(추가 Redis 호출 없이 `find()` 조회에 얹어) 직접 판정해 세션을 지우고 401을 던진다. 실패 사유는 세분화하지 않는다(#26/#29와 같은 원칙 — 거의 발생하지 않는 케이스에 과한 대비를 하지 않기로 함, 사람 확인).
- 세션 값(Redis String+JSON, `session:{sessionId}`)은 **`memberId`, `createdAt`**(#562, 절대 수명 상한 판정용) 두 필드뿐이다. `expiresAt`·`role`은 한때 저장했으나 읽는 코드가 0곳이라 없앴다 — 역할·활성 상태 확인은 항상 회원 조회 이후 최신 DB 상태로 한다. `createdAt`은 `extend()`(EXPIRE)가 값 자체를 건드리지 않으므로 슬라이딩과 무관하게 최초 로그인 시각을 유지한다.
...
- **위치 전송 주기·임계값**(#81, BUSY 간격은 #391로 갱신): BUSY는 고정 주기가 아니라 최소 0.5초 간격으로 "최소 이동 20m 또는 정지 120초"일 때만 전송(#391) / AVAILABLE·UNAVAILABLE은 전송 자체가 없다(#338 — 위 상태 매핑 표의 "없음(#342)"과 동일 사실). 최대 속도 50 m/s, 정확도 상한 100m, 허용 과거 60초·미래 5초, Redis 최신 위치 TTL 10분. **이 값들은 지금 클라이언트(안드로이드)만 쓴다** — 서버측 필터(`LocationAcceptancePolicy`)는 #297에서 제거 후 안 되살렸다. 다시 만들면 두 값이 같아야 한다.
...
- **`RedisMessageListenerConfig.trackingEventExecutor`는 가상 스레드 + `Semaphore(1000)`이다**(#566, 2026-08-18). 예전엔 플랫폼 스레드 고정 풀(4)이라 느린 SSE 클라이언트가 풀 크기만큼만 있어도 이 인스턴스의 모든 배송이 막혔다 — 실측(느린 클라이언트 5개 재현)으로 최대 60초 동안 무관한 위치 갱신 73,956건이 큐 포화로 유실됨을 확인(`docs/loadtest/2026-08-16-sse-fanout-hol-blocking-60s-evidence.md`). 가상 스레드로 메시지마다 별도 스레드를 배정해 이 격리 문제를 없앴고, 동시성 상한은 **넌블로킹 `tryAcquire()`**(사람 확인 — `acquire()`로 바꾸면 안 된다, Redis 구독 스레드 자체가 멈춘다)로 유지한다. 상한 초과 시 큐에 안 쌓고 그 자리에서 드롭하며 `sse.fanout.dropped`(Counter)·`sse.fanout.in_flight`(Gauge)로 관측한다. `1,000`은 기존 큐 상한을 그대로 옮긴 값이라 실측 근거는 약함(확인 필요 항목).
- **HikariCP(DB 커넥션 풀)는 위 이유로 건드리지 않는다**(#566). 풀 크기(10)는 인위적 제한이 아니라 실제 DB 커넥션 개수 제한이라 가상 스레드로 없앨 대상이 아니다. 가상 스레드 전환 시 우려되는 JDBC 드라이버의 핀닝(`synchronized` 블록에서 carrier thread를 못 놓는 문제, JDK 24 이전)도 실측(`-Djdk.tracePinnedThreads=full`, 730,801건 처리, 핀닝 0건)으로 현재 버전(mysql-connector-j 9.7.0, HikariCP 7.0.2, JDK 21.0.11)·쿼리 패턴에서는 확인되지 않았다 — 조건부 확인이라 드라이버 버전 변경이나 느린 쿼리 추가 시 재검증 필요(`docs/loadtest/2026-08-17-hikari-virtual-thread-pinning-evidence.md`).
...
- **`trackingEventExecutor`의 `Semaphore(1000)`이 실측 근거가 없다**(#566). 기존 큐 상한을 그대로 옮긴 값이고, 정상 트래픽의 동시 처리량 기준선을 재지 않았다. `slow-sse-client.py` 재현 실험(느린 클라이언트 다수 + 상한 소진 시 앱 생존 확인)도 새 구현으로 아직 재실행 안 함 — 부하테스트로 확인 후 값 조정 여부 판단.
