...
- **요금 정책(`fare_policy`)의 실제 값이 확정돼 있지 않다**(#510). Flyway(V8)는 스키마만 관리하고, 실제 값을 넣는 시드는 로컬 개발용(`reset-and-seed-local.sql`, 기본요금 5,000+km당 1,000+할증 0~2,000)뿐이라 배포 환경에 어떤 정책이 적용될지 코드베이스만으로는 알 수 없다. 라이더 콜 목록의 "최소 요금" 필터 프리셋(10,000/30,000/50,000P)은 실제 서비스 감각으로 잡은 잠정값이라, 요금 정책이 확정되면 다시 조정해야 한다.
