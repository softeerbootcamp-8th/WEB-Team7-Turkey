...
- **경로 탐색 클라이언트가 `common/routing` 에 생겼다(#420, 2026-08-07) — 구현체가 OSRM 에서
  카카오모빌리티로 바뀌었다(#431, 2026-08-09).** 카카오 API 사용 권한 심사가 끝나 확정됐다(사람 확인).
  `RoutingClient` 인터페이스·호출자(`DeliveryRouteEstimator`, #421)는 무변경 — 인터페이스로 끊어 둔
  것이 실제로 어댑터 교체만으로 끝났다. `OsrmRoutingClient`·배포 OSRM 서버(#416)는 코드에서
  제거했다(인프라 자체를 걷어낼지는 별도 미결, 아래). 확정된 것: 타임아웃 연결 1초 / 읽기 2초
  (#420 의 300ms/700ms 는 VPC 내부 전제라 공인망 HTTPS 에는 짧다고 판단해 늘림, 사람 확인),
  재시도 없음, 실패는 예외 대신 빈 값, 연속 3회 실패 시 호출 스킵(대기 1초 → 2배씩 → 30초 상한,
  성공 시 초기화 — `RoutingFailureBackoff` 그대로 재사용), 경로 좌표(`Route.path`)는 받되
  `summary` 파라미터를 지정하지 않는다(사람 확인, #422 대비). 남은 미결:
  - **4xx 를 실패로 세는 판정이 OSRM 과 정반대로 뒤집혔다.** OSRM 은 "경로 없음"을 400 으로 줘서
    4xx 를 장애로 안 셌지만, 카카오는 "경로 없음"을 `HTTP 200 + result_code != 0` 으로 주고 4xx
    (401 키 불량·403 권한·429 쿼터 초과)는 전부 진짜 장애라 실패로 센다 — 특히 429 는 과금 API 를
    계속 두드리는 셈이라 물러서는 게 맞다(스스로 판단, worklog 2026-08-09-431 참고). 서비스가
    커져 이 구분이 또 바뀔 수 있다는 걸 다음에도 의식할 것
  - 타임아웃 값이 **실측 없이 정한 잠정값**이다. 부하 테스트와 화면(#422)에서 재검토
  - **로컬·CI 어디에도 실제 카카오 API 에 붙는 테스트가 없다**(호출당 과금이라 사람 확인으로
    의도적으로 막음). 응답 스키마는 공식 문서 조사로만 확인했고, **실제 응답과의 일치는 배포 후
    첫 호출에서만 확인된다** — 스키마가 다르면 조용히 빈 값만 나가고 예외는 안 뜬다(로그로 확인 필요)
  - `result_code` 의 전체 값 표(0=성공 외의 개별 사유)를 확인하지 못했다. 지금은 "0 이 아니면
    실패"로만 다뤄 동작엔 문제없지만, 사유별 로그·지표가 필요해지면 공식 문서나 실제 응답 관찰로
    다시 확인해야 한다
  - 429 응답에 `Retry-After` 등 재시도 안내가 오는지, 온다면 고정 백오프 대신 그 값을 쓸지 미결
  - 카카오 콘솔의 실제 쿼터·요금제 수치를 이 작업에서 확인하지 못했다. 배포 전 콘솔 확인 필요
  - 경로 결과 캐시가 없고, 실패 사유를 호출자에게 알리지 않는다(전부 빈 값 하나로 뭉갬, OSRM 시절과
    동일)
  - 백오프 카운트는 **인스턴스 로컬**이다. 인스턴스가 늘면 각자 세 번씩 확인한다(의도)
  - 배포된 OSRM 서버(#416) 인프라를 걷어낼지, 다른 용도(카카오 장애 시 폴백 등)로 재사용할지 미결
    — 이번 이슈는 애플리케이션 코드만 다뤘다
- **추적 스냅샷이 ETA 를 싣는다(#421, 2026-08-07).** 출발점은 Redis 최신 위치, 도착점은 상태별로
  ...
  **예상 경로 좌표는 싣지 않는다**(사람 확인, 2026-08-07 — 한 번 구현했다가 걷어냈다). 고객에게
  ...
  - **경로 결과 캐시가 없어 조회마다 라우팅 API 를 부른다.** 백업 폴링 1분 주기 × 동시 추적 수만큼 호출되고,
    #431 부터는 호출당 과금이라 이 비용이 그대로 청구액이다
  ...
  - `DeliveryTrackingQueryService.getTracking` 에서 **`@Transactional` 을 걷어냈다** — 외부 HTTP
    호출(#431 기준 최대 3초)이 DB 커넥션을 잡지 않게 하려는 것이고, 대신 지연 로딩을 쓸 수 없어
