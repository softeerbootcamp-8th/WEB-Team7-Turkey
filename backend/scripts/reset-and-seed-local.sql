...
-- 클라이언트 접속 문자셋을 명시한다. mysql 클라이언트는 --default-character-set 없이
-- 접속하면 latin1 계열로 협상되는 경우가 있어, 이 파일의 UTF-8 한글 리터럴이 그 문자셋으로
-- 잘못 해석돼 저장될 수 있다(예: "요청" -> "ìš”ì²­" 로 깨져서 들어감). 실행 커맨드에 플래그를
-- 붙이는 방식은 매번 기억해야 해서 누락되기 쉬우므로, 파일 자체에서 강제한다.
SET NAMES utf8mb4;
