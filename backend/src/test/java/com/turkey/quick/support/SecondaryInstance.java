 ...
 *
 * <p>두 컨텍스트는 서로 다른 {@code BeanFactory} 를 가지므로 {@code TrackingEmitterRegistry} 가
 * 컨텍스트당 하나씩 생긴다 — 그래서 "인스턴스 로컬"이 자연히 재현된다. <b>레지스트리를
 * {@code static} 으로 바꾸면 두 컨텍스트가 같은 맵을 공유해 테스트가 통과하면서 아무것도 증명하지
 * 못하므로</b>, 테스트가 두 레지스트리의 동일성과 B 가 비어 있음을 함께 단언한다.
 *
