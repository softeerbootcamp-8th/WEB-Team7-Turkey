 ...
 *
 * <p>순서를 타입으로 고정하는 것이 이 record 의 존재 이유다. OSRM 요청 경로는 {@code [경도, 위도]}
 * 순인데, 서울 좌표는 (37.5, 127.0) 이라 뒤집어도 <b>둘 다 유효 범위에 들어가</b> 예외가 나지 않는다
 * — 지도에 엉뚱한 위치가 조용히 찍힐 뿐이다(#423 「알려진 리스크」). 그래서 뒤집는 코드는
 * {@link OsrmRoutingClient} 안에만 두고, 그 밖의 코드는 이 타입만 주고받는다.
 *
