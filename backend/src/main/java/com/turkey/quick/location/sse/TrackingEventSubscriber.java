...
/**
 * Redis Pub/Sub 으로 팬아웃된 위치 이벤트를 받아 <b>이 인스턴스가 들고 있는</b> 연결로만 보낸다(#78).
 *
 * <p>모든 인스턴스가 같은 패턴을 구독하므로 모든 이벤트를 받는다. 그중 자기 레지스트리에 연결이
 * 있는 주문만 실제로 전송하고, 나머지는 맵 조회 한 번으로 끝난다 — 그게 팬아웃의 전부다.
...
public class TrackingEventSubscriber implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        Optional<Long> orderId = TrackingChannel.orderIdOf(channel);
        if (orderId.isEmpty()) {
        ...
        for (TrackingConnection connection : registry.connectionsOf(orderId.get())) {
            // 연결 하나하나를 독립적으로 처리한다. TrackingConnection 이 전송 예외를 boolean 으로
            // 바꿔 주므로, 한 연결이 죽어 있어도 뒤에 있는 정상 연결들이 이벤트를 받는다.
            if (!connection.sendLocation(json)) {
                subscriptionService.disconnect(connection, "SEND_FAILED");
            }
        }
    }
}
