...
public class RiderLocationService {
     ...
     * 비용이 늘지 않는다.
     *
     * <p><b>조회를 없애고 라이더→배송 매핑을 캐시하지 않는 이유</b>: 그 조회의 상태 조건이
     * <b>안전장치</b>다. 배송이 완료·취소되면 결과가 비어 발행이 멈추고 고객의 스트림은 조용해진다.
     * 캐시는 무효화를 한 번 놓치는 순간 그 라이더의 <i>다음</i> 배송 경로가 이전 고객에게 흘러가고,
     * 하필 그 무효화 지점(배송 완료 시 연결 종료)이 아직 구현되지 않았다(#80).
     *
     * <p><b>조회 실패가 위치 갱신을 실패시키지 않는다.</b> 이 경로에 MySQL 의존이 새로 생겼으므로,
     * DB 장애가 배차 후보 갱신(#83)과 최신 위치 저장까지 같이 죽이지 않도록 예외를 삼킨다. 전달은
     * at-most-once 이고 다음 전송(5초)이 복구한다.
     *
     * <p>수행 중 배송이 없는 BUSY 라이더는 <b>정합성이 깨진 상태</b>다(운행 상태는 BUSY 인데 배정된
     * 배송이 없다). 조용히 넘기면 원인을 찾을 수 없어 WARN 을 남긴다 — 배송 완료 처리와 라이더 상태
     * 전이가 어긋났을 때 여기로 드러난다.
     */
