...
public class RiderLocationService {
     ...
     * 가능성을 원천 차단한다.
     *
     * <p><b>조회 실패가 위치 갱신(syncRedisState)을 실패시키지 않는다.</b> 이 경로에 MySQL
     * 의존(조회)이 새로 생겼으므로, DB 장애가 배차 후보 갱신(#83)과 최신 위치 저장까지 같이 죽이지
     * 않도록 예외를 삼킨다. 전달은 at-most-once 이고 다음 전송(5초)이 복구한다.
     *
