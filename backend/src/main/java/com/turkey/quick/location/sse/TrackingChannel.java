...
public final class TrackingChannel {
    ...
    /**
     * 모든 주문 채널을 한 번에 구독하는 패턴({@code PSUBSCRIBE}).
     *
     * <p><b>주문별로 구독·해제하지 않고 패턴 하나로 받는 이유</b>(계획과 달라진 지점):
     * <ul>
     *   <li>주문별 동적 구독은 refcount 가 필요하고, 같은 주문이 1→0 과 0→1 을 동시에 겪을 때
     ...
     * (BUSY 라이더 수 / 5초) × 인스턴스 수 만큼의 맵 조회이고, 대부분은 연결이 없어 즉시 끝난다.
     * 인스턴스를 크게 늘려 이 비용이 문제가 되면 <b>채널 이름이 이미 주문별이므로</b> 값 형식을
     * 바꾸지 않고 주문별 구독으로 전환할 수 있다.
     */
