 ...
 */
public record AuthenticatedCustomer(Long memberId, String loginId, String name) {

    public static AuthenticatedCustomer from(Member member) {
        return new AuthenticatedCustomer(member.getId(), member.getLoginId(), member.getName());
    }
}
