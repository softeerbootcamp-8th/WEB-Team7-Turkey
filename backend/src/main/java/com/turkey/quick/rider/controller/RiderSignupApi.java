...
public interface RiderSignupApi {

    @Operation(
            operationId = "riderSignup",
            summary = "라이더 회원가입",
            description = "휴대전화 인증을 완료한 라이더의 계정과 라이더 프로필(초기 운행 상태 UNAVAILABLE)을 생성한다. "
                    + "로그인 ID·휴대전화 번호가 중복이면 409, 인증 미완료·형식 오류·존재하지 않는 약관 ID는 400을 반환한다."
    )
    ApiResponse<RiderSignupResponse> signup(@Valid RiderSignupRequest request);
}
