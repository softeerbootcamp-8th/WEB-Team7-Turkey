...
/**
 * 라이더 로그아웃(이슈 #51). 고객 로그아웃(#28)과 뼈대는 같지만, 운행 상태 확인·전이가 도메인 규칙이라
 * 그 판단을 {@link RiderLogoutService}로 위임한다. 컨트롤러는 쿠키 추출과 만료 응답만 담당한다.
 *
 * <p>인증 처리를 인터셉터에 맡기지 않는 이유: 이미 만료·삭제된 세션에도 로그아웃 완료(200)를 줘야 하는데,
 * {@code RiderSessionInterceptor}는 그런 경우 401을 던진다. 그래서 이 경로는 {@code RiderWebMvcConfig}의
...
public class RiderLogoutController {
    ...
    public ApiResponse<Void> logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        // BUSY면 여기서 409가 던져져 아래 쿠키 만료가 실행되지 않는다 — 라이더는 로그인 상태로 남는다.
