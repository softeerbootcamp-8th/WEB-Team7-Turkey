...
- 완료 또는 취소 시 해당 배송의 연결을 종료한다. **아직 능동적 종료는 구현되지 않았다**(#80) —
  채널 키가 주문이라 완료되면 발행이 멈춰 스트림이 조용해지지만, 연결 자체는 타임아웃까지 열려 있다.
- **타임아웃·heartbeat·중복 연결 수치는 #77 에서 확정했다**(사람 확인, 2026-07-30):
  emitter 타임아웃 **5분** / heartbeat **15초** / 동일 주문 연결 한도 **3** / stale 임계 **45초** /
  서버가 `retry: 3000` 지정. 값은 `location/sse/TrackingStreamPolicy` 한 곳에 있다.
  - 깨지면 안 되는 순서: `heartbeat 간격 < CloudFront 오리진 응답 타임아웃 < ALB idle timeout`.
    CloudFront 는 스트리밍 응답에서 **패킷 사이 간격**에 타임아웃을 적용한다. 오리진 응답 타임아웃
    상한 120초·keep-alive 300초를 확인했고(사람 확인), ALB 는 아직 도입 전이다.
  - **heartbeat 가 keep-alive 만 하는 것이 아니다.** 같은 tick 에서 죽은 연결을 탐지하고
    (SSE 는 쓰기를 시도할 때만 끊김을 알 수 있다) 연결 집계의 생존 시각을 갱신한다. 그래서
    heartbeat 를 빼면 연결 한도가 "동시 3개"가 아니라 "타임아웃당 3개"가 된다.
  - 재연결은 **최신 스냅샷으로 복구**하고 이벤트를 재생하지 않는다(#79). 그래서 `id:` 필드를
    보내지 않는다 — 보내면 지키지 못할 약속이 된다.
- 이벤트 전달은 **Redis Pub/Sub 팬아웃**이다(2026-07-29 결정,
  [Discussion #246](https://github.com/softeerbootcamp-8th/WEB-Team7-Turkey/discussions/246)).
  위치를 처리한 인스턴스가 발행하고, 모든 인스턴스가 구독해 해당 주문의 연결을 들고 있는 인스턴스만
  전송한다. 전달은 at-most-once다 — 유실된 이벤트는 다음 위치 이벤트와 재연결 시 스냅샷이 복구한다.
- 중복 연결 수 제한은 **인스턴스별로 셀 수 없다** — 같은 주문의 연결이 여러 인스턴스에 흩어지므로
  Redis 카운터로 센다.
- 타임아웃·heartbeat 는 각 인스턴스가 **자기가 들고 있는 연결에 대해서만** 수행한다. 전역 조정이
  필요 없고, 인스턴스가 늘어도 그대로 옳다.
