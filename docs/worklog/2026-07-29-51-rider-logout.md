...
이미 만료·삭제된 세션에도 200으로 완료 처리한다(멱등). 상태 판단이 도메인 규칙이라 #28과 달리
서비스 계층(`RiderLogoutService`)을 뒀다.
...
- **세션 삭제를 트랜잭션 `afterCommit`로 지연**: 상태 전이(MySQL)와 세션 삭제(Redis)는 원자적으로 묶을 수 없다. 세션을 먼저 지운 뒤 상태 커밋이 실패하면 "로그아웃됐지만 여전히 AVAILABLE(배차 후보)"인 라이더가 남는다 — 로그아웃한 라이더에게 배차가 갈 수 있는 정합성 구멍. 그래서 세션 삭제를 `afterCommit`로 미뤄 상태 전이가 확정된 뒤에만 지운다. 커밋 실패 시 세션이 살아있어 재시도 가능. (트랜잭션 동기화가 없는 순수 단위 테스트에서는 동기 삭제로 폴백한다.)
...
- **Redis GEO 배차 후보 스토어**: 미구현 상태 그대로 뒀다 — 이유는 위 「사람이 고른 선택 2」. 후속: GEO 후보 스토어 구현 이슈에서 로그아웃 경로에 GEO 제거를 연결해야 함(미등록).
- **프론트 연동/Orval 재생성**: 이 이슈는 backend 범위. 로그아웃 버튼 화면 연결은 별도 프론트 이슈에서.
- **BUSY afterCommit 롤백의 멀티스레드 재현 테스트**: 단일 요청 경로라 동시성 재현 테스트는 두지 않았다.

## 테스트

| 층 | 파일 | 검증한 것 |
|---|---|---|
| 단위 | `rider/service/RiderLogoutServiceTest.java` | AVAILABLE→UNAVAILABLE+세션삭제 / UNAVAILABLE 상태유지+세션삭제 / BUSY 409 거부·상태·세션 유지 / null·미상 세션 멱등(조회 없음) |
| 통합 | `rider/service/RiderLogoutServiceIntegrationTest.java` | 실제 MySQL에서 AVAILABLE 로그아웃이 UNAVAILABLE로 커밋+afterCommit 세션삭제 / BUSY 409 롤백으로 DB·세션 불변 / UNAVAILABLE 세션만 삭제 |
