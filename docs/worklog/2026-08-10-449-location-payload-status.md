...
- **STATUS 프레임도 기준값(`lastStatusRef`)을 갱신한다 — 리뷰 중 발견한 결함을 고친 것이다.**
  처음 구현은 STATUS 분기에서 기준값을 올리지 않고 빠져나갔다. 그러면 전이 하나에 재조회가
  **두 번** 돈다:

  ```
  t=0     STATUS(PICKED_UP) → 재조회 ①,  기준값은 여전히 MOVING_TO_PICKUP
  t=+3초  위치 프레임(PICKED_UP) → "달라졌다" 판정 → 재조회 ② (불필요)
  ```

  정합성이 깨지는 건 아니지만(재조회는 멱등) 전이마다 REST가 두 번 나간다. STATUS는 빠른 경로,
  위치는 안전망이므로 **둘이 같은 전이를 가리킬 때는 한 번만 반응**해야 한다.

- **그 판정을 `nextTrackingSignal` 순수 함수로 분리했다** — 프레임 종류·상태 유무·첫 프레임 여부로
  분기가 셋이라 인라인으로 두면 회귀를 잡을 수 없다(중복 재조회는 화면에 드러나지 않고 네트워크
  탭에서만 보인다). `frontend/CLAUDE.md`의 "분기가 많은 로직은 순수 함수로 떼어내 테스트한다"는
  관례를 따랐다.

### STATUS 프레임은 여전히 필요한가

위치에 상태가 실리면서 중간 전이에는 두 메커니즘이 겹치게 됐다. 그래도 STATUS를 유지하는 이유:

| | STATUS 프레임 | 위치에 실린 status |
|---|---|---|
| 중간 전이 | 즉시 | 최대 5초 후 |
| `DELIVERING→COMPLETED` | **유일한 수단** | 불가 — 그 순간 위치가 끊긴다 |

즉 겹치는 것은 중간 전이뿐이고, 그건 **의도한 계층**이다(빠른 경로 + 안전망).
