...
대안이었던 "같은 채널 + `type` 필드"가 탈락한 **결정적 이유는 누출을 구조로 막는 것**이다. 같은
채널이면 `TrackingSubscriber` 가 릴레이할지 닫을지 판단해야 하고, 그 판단이 틀리면 `"close"` 가 SSE
`data:` 로 브라우저에 도착한다. 채널을 나누면 리스너가 아예 달라 **그 실수가 성립하지 않는다.**
판별자를 이미 디스패치하는 계층(Redis 패턴 매칭)에 두는 것이기도 하다.

**"구독자는 파싱하지 않는다"를 근거로 앞세우지 않는다.** 확인해 보니 #78(`aec5059`)이 CLAUDE.md 에
쓴 원문은 계약 선언이 아니라 사실 서술이고, 거기서 나온 규칙은 "필드 추가만 허용"이다. 이 결정은
그 서술과 충돌하지 않을 뿐이며, 서버측 순서 가드(디스패처 풀 4의 순서 역전 대비)처럼 파싱이 필요한
요구가 생기면 채널을 더 쪼갤 게 아니라 파싱하는 것이 맞다.

파싱 비용은 근거가 **아니다** — BUSY 라이더 수 ÷ 5초 × 인스턴스 수라 동시 배송 100건이어도 초당
20건이고, 측정 가능한 차이가 아니다.
