...
**계단 램프가 기본 도구다.** 한 번의 런으로 여러 VU 수준을 재면 "어디서 꺾였나"가 한 표에
드러난다. `STEPS=n` 을 주면 `MAX_VU/n` 간격으로 n 단을 만들고 각 단을 `HOLD_SECONDS` 유지한다.
**한 단은 90초 이상**이어야 한다(아래 이유). `collect.py --steps` 가 VU 타임라인에서 유지 구간을
찾아 단계별 표를 만들므로, 단 경계를 따로 기록해 둘 필요는 없다.

**측정 구간(계단이면 한 단)은 90초 이상으로 잡는다.** Prometheus 스크레이프가 15초라, 구간이 짧으면
